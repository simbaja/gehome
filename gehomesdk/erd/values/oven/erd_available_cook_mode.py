...
class ErdAvailableCookMode(enum.Enum):
    ...
    documentation, it doesn't even look right.  For now, until I can find some newer documentation (or can
    derive it from various appliance testing), I'll just assume both are right and include the largest subset of
    options possible.  I trust the documentation from the GE Maker site more than the app, but it doesn't have
    the newer modes unfortunately.  See to12-interface.docx for more information.
    """
    # From GE Maker Site
